-- +goose Up
-- Some installed databases already carry a historical version-5 marker without
-- the manual-bank objects. Append a forward repair; never rewrite that marker
-- or alter posted financial history. Fresh version-5 installations are safe too.
CREATE UNIQUE INDEX IF NOT EXISTS journal_manual_bank_payment_unique
ON journal_transactions(external_reference)
WHERE source_system='manual-bank-reconciliation';

-- +goose StatementBegin
CREATE OR REPLACE FUNCTION enforce_manual_bank_posting() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS manual_bank_posting_checked ON journal_transactions;
...
-- +goose StatementBegin
DO $$ BEGIN
 RAISE EXCEPTION 'Manual bank integrity safeguards cannot be removed by rollback';
END $$;
-- +goose StatementEnd
